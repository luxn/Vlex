...
namespace vlex {
	namespace graphics {
		...
		GLuint Shader::load() {
			...
			std::string vertSourceString = utils::readFileAsString(m_VertPath);
			std::string fragSourceString = utils::readFileAsString(m_FragPath);
			...
			if (result == GL_FALSE) {
				...
				std::cerr << "Failed to compile vertex shader: " << &error[0] << std::endl;
			...
			if (result == GL_FALSE) {
				...
				std::cerr << "Failed to compile fragment shader: " << &error[0] << std::endl;
